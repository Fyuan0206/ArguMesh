...
**Distribution status — published.** `v3.2.5` is live on [GitHub Releases](https://github.com/Fyuan0206/ArguMesh/releases/tag/v3.2.5) with a Windows NSIS installer attached (2026-09-24, after the user re-raised it), and both READMEs carry a **Download / 下载安装** section pointing at it. The decision record in `docs/DISTRIBUTION-RESEARCH-2026-09-20.md` and the build recipe in `docs/DEPLOYMENT.md` §3 both still hold. Three facts stated in the release notes **and** in the README download section, which any future download entry must repeat: **the installer ships an empty database** (AI config is not in it — re-enter it on the Settings page), **the build is unsigned** (SmartScreen will warn), and **the app has no auth** (never expose its port to an untrusted network). Still unimplemented: code signing and `.github/workflows/release.yml` — publishing is manual (commit → tag → `gh release create` with a freshly built installer).
