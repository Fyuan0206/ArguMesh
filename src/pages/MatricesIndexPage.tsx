  ...
  return <div className="route-page">
    ...
    {creating ? <form className="surface-card matrix-creator" onSubmit={(event) => void submit(event)}>
      ...
      <div className="matrix-form-grid">
        ...
        <fieldset className="matrix-paper-picker"><legend>纳入论文 <strong>{selectedPaperIds.length} / {projectPapers.length}</strong><small>默认全选项目论文;新建只纳入此时勾选的论文</small></legend>{projectPapers.length ? <div>{projectPapers.map((paper) => <label key={paper.id}><input type="checkbox" checked={selectedPaperIds.includes(paper.id)} onChange={() => togglePaper(paper.id)} /><span><strong>{paper.title}</strong><small>{paper.authors} · {paper.venue} {paper.year}</small></span></label>)}</div> : <p>当前项目还没有论文。请先到项目文献库添加论文。</p>}</fieldset>
      </div>
    ...
    {visibleMatrices.length === 0 ? <EmptyState icon={<GridFour />} title="还没有证据矩阵" description="矩阵自动使用项目里的论文——新建时默认纳入项目全部论文。" action={<button className="primary" onClick={() => setCreating(true)}><Plus /> 新建矩阵</button>} /> : null}
  </div>;
}